"""Offline phase smoke tests using dummy images and tiny random Qwen weights."""
...
def smoke(root: Path, phase: int) -> None:
    ...
    logging.getLogger(__name__).warning(
        "Phase %s smoke passed. Random-model metrics are NOT research results.", phase
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--phase", type=int, default=6, choices=range(1, 7))
    args = parser.parse_args()
    with tempfile.TemporaryDirectory(prefix="htr-smoke-") as directory:
        smoke(Path(directory), args.phase)
